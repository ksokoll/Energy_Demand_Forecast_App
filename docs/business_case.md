...
In a consulting engagement, this architecture would be the starting point for a client project, not the end product. The domain-specific adaptation typically involves three steps:

1. **Swap the data source.** Replace the energy dataset with the client's data. The `DataStore` protocol makes this a single implementation change.
2. **Adjust the feature set.** Keep calendar and lag features as-is. Replace weather interactions with domain-relevant drivers (e.g., promotional events for retail, flu incidence for healthcare).
3. **Tune the forecast horizon.** Change `FORECAST_HORIZON` in config. All downstream leakage guards and lag constraints update automatically.

The feature pipeline, serving architecture, testing strategy, and deployment setup remain unchanged. That reusability is the point.